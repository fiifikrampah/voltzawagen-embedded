...
int SoftwareSerial::available() {
    //Serial.print("SoftwareSerial::available(): Inside func\n\r");
    if (!m_rxValid) { return 0; }
    //Serial.print("SoftwareSerial::available(): m_rxValid is true\n\r");

    rxBits();
    int avail = m_buffer->available();
    if (!avail) {
        //Serial.print("SoftwareSerial::available: Before optimistic yield\n\r");
        optimistic_yield(10000);
        //Serial.print("SoftwareSerial::available: After optimistic yield\n\r");
    }
    //Serial.print("SoftwareSerial::available(): Returning Avail: \n\r");
...
size_t SoftwareSerial::write(uint8_t byte) {
    //Serial.print("SoftwareSerial::write -> Writing byte: \n\r");
    //Serial.println(byte);
    return write(&byte, 1);
}
